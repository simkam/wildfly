...
/**
 * Validates that web applications within an ear can share sessions if configured appropriately, but sessions aren't
 * shared between nodes when they are not configured to use distributable session manager.
 *
 * It runs only on single node by utilizing the fact that a distributable web application preserves its web sessions
 * across server restarts, a non-distributable web application does not.
 */
@RunWith(Arquillian.class)
public class DisabledSharedSessionTestCase {

    private static final String MODULE_PREFIX = DisabledSharedSessionTestCase.class.getSimpleName() + '-';
    ...
    private static Archive<?> getDeployment() {
        ...
        ear.addAsManifestResource(DisabledSharedSessionTestCase.class.getPackage(), "jboss-all_no_distributable.xml", "jboss-all.xml");
        return ear;
    }
